...
type MaterialRepository interface {
	...
	DeleteReportingReference(ctx context.Context, db domain.DBTX, id int64) domain.MaterialDeleteResult
}
...
func (materialRepository) DeleteReportingReference(ctx context.Context, db domain.DBTX, id int64) domain.MaterialDeleteResult {
	_, err := db.ExecContext(ctx, `DELETE FROM materials WHERE id=?`, id)
	return domain.MaterialDeleteResult{Referenced: isFKConstraint(err), Err: err}
}
