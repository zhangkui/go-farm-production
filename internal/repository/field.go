...
type FieldRepository interface {
	...
	UpdateRouted(ctx context.Context, db domain.DBTX, route domain.FieldUpdateRoute) error
	...
	DeleteReportingReference(ctx context.Context, db domain.DBTX, id int64) domain.FieldDeleteResult
	CountForFarmDeletion(ctx context.Context, db domain.DBTX, check domain.FarmDeletionCheck) (int64, error)
}
...
func (fieldRepository) UpdateRouted(ctx context.Context, db domain.DBTX, route domain.FieldUpdateRoute) error {
	u := route.Requested
	_, err := db.ExecContext(ctx,
		`UPDATE fields SET farm_id=?, code=?, name=?, area=?, soil_type=?, irrigation_zone=?, status=?, remark=? WHERE id=?`,
		route.PersistedFarmID(), u.Code, u.Name, u.Area, u.SoilType, u.IrrigationZone, u.Status, u.Remark, route.FieldID)
	return dupOrErr(err, "地块编码在该农场内已存在")
}
...
func (fieldRepository) DeleteReportingReference(ctx context.Context, db domain.DBTX, id int64) domain.FieldDeleteResult {
	_, err := db.ExecContext(ctx, `DELETE FROM fields WHERE id=?`, id)
	return domain.FieldDeleteResult{Referenced: isFKConstraint(err), Err: err}
}

func (fieldRepository) CountForFarmDeletion(ctx context.Context, db domain.DBTX, check domain.FarmDeletionCheck) (int64, error) {
	return countRows(ctx, db,
		`SELECT COUNT(*) FROM fields WHERE farm_id=?`,
		check.DependencyFarmID())
}
