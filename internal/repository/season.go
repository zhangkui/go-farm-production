...
type SeasonRepository interface {
	...
	CreateWindow(ctx context.Context, db domain.DBTX, s *domain.Season, window domain.SeasonCreateWindow) (int64, error)
	Update(ctx context.Context, db domain.DBTX, id int64, u *domain.SeasonUpsert) error
	UpdateProjected(ctx context.Context, db domain.DBTX, p domain.SeasonUpdateProjection) error
...
func (seasonRepository) CreateWindow(ctx context.Context, db domain.DBTX, s *domain.Season, window domain.SeasonCreateWindow) (int64, error) {
	q := `INSERT INTO seasons (code, name, start_date, end_date, status) VALUES (?, ?, ?, ?, ?)`
	id, err := execInsert(ctx, db, q, s.Code, s.Name, window.Start, window.End, s.Status)
	if err != nil {
		return 0, dupOrErr(err, "季次编码已存在")
	}
	return id, nil
...
func (seasonRepository) UpdateProjected(ctx context.Context, db domain.DBTX, p domain.SeasonUpdateProjection) error {
	_, err := db.ExecContext(ctx,
		`UPDATE seasons SET code=?, name=?, start_date=?, end_date=?, status=? WHERE id=?`,
		p.Code, p.PersistedName(), p.StartDate, p.EndDate, p.PersistedStatus(), p.ID)
	return dupOrErr(err, "季次编码已存在")
}
