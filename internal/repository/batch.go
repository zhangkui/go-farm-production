...
type InputBatchRepository interface {
	...
	UpdateWithRemaining(ctx context.Context, db domain.DBTX, id int64, u *domain.InputBatchUpsert, remaining domain.Decimal) error
...
func (inputBatchRepository) UpdateWithRemaining(ctx context.Context, db domain.DBTX, id int64, u *domain.InputBatchUpsert, remaining domain.Decimal) error {
	q := `UPDATE input_batches SET material_id=?, batch_no=?, quantity=?, remaining_qty=?, purchase_date=?, expiry_date=?, purchase_price=?, supplier=?, status=? WHERE id=?`
	_, err := db.ExecContext(ctx, q, u.MaterialID, u.BatchNo, u.Quantity, remaining, u.PurchaseDate, u.ExpiryDate, u.PurchasePrice, u.Supplier, u.Status, id)
	return dupOrErr(err, "批号在该物料下已存在")
}
...
func (inputBatchRepository) IncreaseStock(ctx context.Context, db domain.DBTX, id int64, qty domain.Decimal) (domain.Decimal, error) {
	if _, err := db.ExecContext(ctx,
		`UPDATE input_batches SET remaining_qty = remaining_qty + ?, status = ? WHERE id=?`,
		qty, domain.BatchStatusActive, id); err != nil {
		return 0, err
	}
	var remaining domain.Decimal
	if err := db.QueryRowContext(ctx, `SELECT remaining_qty FROM input_batches WHERE id=?`, id).Scan(&remaining); err != nil {
...
func (inputBatchRepository) ListExpiring(ctx context.Context, db domain.DBTX, onOrBefore string) ([]*domain.InputBatch, error) {
	...
	rows, err := db.QueryContext(ctx, q, onOrBefore)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
