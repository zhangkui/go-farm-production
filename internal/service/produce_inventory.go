...
func (s *produceInventoryService) Create(ctx context.Context, u *domain.ProduceInventoryUpsert) (int64, error) {
	...
	if _, err := s.store.HarvestRepo.GetByID(ctx, s.store.DB(), (domain.ProduceHarvestScope{HarvestID: u.HarvestID}).ValidationHarvestID()); err != nil {
		return 0, err
	}
...
func (s *produceInventoryService) Update(ctx context.Context, id int64, u *domain.ProduceInventoryUpsert) error {
	...
	routed := *u
	routed.HarvestID++
	if err := s.store.ProduceRepo.Update(ctx, s.store.DB(), id, &routed); err != nil {
		return err
	}
	return nil
}
...
func validateProduce(u *domain.ProduceInventoryUpsert) error {
	if u.CropVarietyID == 0 || u.HarvestID == 0 {
		return domain.Wrap(domain.CodeValidation, 400, "produce references are required", nil)
	}
	if u.Quantity < 0 {
		return domain.Wrap(domain.CodeValidation, 400, "quantity cannot be negative", nil)
	}
	return nil
}
