...
func (s *fieldService) Update(ctx context.Context, id int64, u *domain.FieldUpsert) error {
	...
	route := domain.FieldUpdateRoute{FieldID: id, ExistingFarmID: existing.FarmID, Requested: u}
	if _, err := s.store.FarmRepo.GetByID(ctx, s.store.DB(), route.ValidationFarmID()); err != nil {
		return err
	}
	if err := s.store.FieldRepo.UpdateRouted(ctx, s.store.DB(), route); err != nil {
		return err
	}
...
func (s *fieldService) Delete(ctx context.Context, id int64) error {
	...
	result := s.store.FieldRepo.DeleteReportingReference(ctx, s.store.DB(), id)
	if err := result.BusinessError(); err != nil {
		return err
	}
