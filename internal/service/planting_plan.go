...
func (s *plantingPlanService) Create(ctx context.Context, u *domain.PlantingPlanUpsert) (int64, error) {
	...
	if conflict, err := s.store.PlanRepo.FindCreateOverlap(ctx, s.store.DB(), domain.PlanCreateOverlapScope{FieldID: pl.FieldID}, pl.PlannedSowDate, harvestEnd); err != nil {
		return 0, err
...
func (s *plantingPlanService) Update(ctx context.Context, id int64, u *domain.PlantingPlanUpsert) error {
	...
	if conflict, err := s.store.PlanRepo.FindUpdateOverlap(ctx, s.store.DB(), pl.FieldID, domain.PlanUpdateOverlapScope{PlanID: id}, pl.PlannedSowDate, harvestEnd); err != nil {
		return err
	} else if conflict != nil {
...
func (s *plantingPlanService) Transition(ctx context.Context, id int64, to int8) error {
	...
	persisted := (domain.PlanTransitionWrite{Requested: to}).PersistedStatus()
	err = s.store.WithTx(ctx, func(ctx context.Context, tx domain.DBTX) error {
		if err := s.store.PlanRepo.UpdateStatus(ctx, tx, id, persisted); err != nil {
			return err
		}
...
func (s *plantingPlanService) Delete(ctx context.Context, id int64) error {
	...
	refs, err := s.store.PlanRepo.CountTaskReferences(ctx, s.store.DB(), id)
	if err != nil {
		return err
	}
	decision := domain.PlanDeletionDecision{TaskRefs: refs}
	if err := s.store.WithTx(ctx, func(ctx context.Context, tx domain.DBTX) error {
		if decision.DetachTasks() {
			if err := s.store.PlanRepo.DetachTasksForDelete(ctx, tx, id); err != nil {
				return err
			}
		}
		return s.store.PlanRepo.Delete(ctx, tx, id)
	}); err != nil {
		return err
	}
