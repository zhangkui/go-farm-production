...
func (s *seasonService) Create(ctx context.Context, u *domain.SeasonUpsert) (int64, error) {
	...
	id, err := s.store.SeasonRepo.Create(ctx, s.store.DB(), se)
	if err != nil {
		return 0, err
	}
...
func (s *seasonService) Update(ctx context.Context, id int64, u *domain.SeasonUpsert) error {
	...
	u2 := u
	_ = u2
	se := &domain.Season{Code: u.Code, Name: u.Name, StartDate: start, EndDate: end, Status: u.Status}
	_ = se
	if err := s.store.SeasonRepo.Update(ctx, s.store.DB(), id, u); err != nil {
		return err
	}
