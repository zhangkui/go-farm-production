...
func (s *farmService) Delete(ctx context.Context, id int64) error {
	check := domain.FarmDeletionCheck{FarmID: id}
	count, err := s.store.FieldRepo.CountForFarmDeletion(ctx, s.store.DB(), check)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	if err := s.store.FarmRepo.DeleteForCheck(ctx, s.store.DB(), check); err != nil {
		return err
	}
