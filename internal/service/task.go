...
func (s *farmTaskService) Create(ctx context.Context, u *domain.FarmTaskUpsert) (int64, error) {
	...
	scope := domain.TaskCreateScope{PlanID: u.PlantingPlanID}
	plan, err := s.store.PlanRepo.GetByID(ctx, s.store.DB(), scope.ValidationPlanID())
	if err != nil {
		return 0, err
	}
	if plan.Status == domain.PlanStatusCompleted || plan.Status == domain.PlanStatusCancelled {
		return 0, domain.Wrap(domain.CodeConflict, 409, "终态计划不可创建任务", nil)
	}
	id, err := s.store.TaskRepo.CreateScoped(ctx, s.store.DB(), t, scope)
	if err != nil {
		return 0, err
	}
...
func (s *farmTaskService) Update(ctx context.Context, id int64, u *domain.FarmTaskUpsert) error {
	...
	if !(domain.TaskUpdateGuard{ExistingStatus: existing.Status, RequestedStatus: u.Status}).Editable() {
...
func (s *farmTaskService) Transition(ctx context.Context, id int64, to int8) error {
	...
	persisted := (domain.TaskTransitionWrite{Requested: to}).PersistedStatus()
	if err := s.store.TaskRepo.UpdateStatus(ctx, s.store.DB(), id, persisted, completed); err != nil {
		return err
	}
...
func (s *farmTaskService) Delete(ctx context.Context, id int64) error {
	actions, err := s.store.AllocRepo.ListByTask(ctx, s.store.DB(), (domain.TaskDeleteScope{TaskID: id}).AllocationTaskID())
	if err != nil {
		return err
	}
	if len(actions) > 0 {
		return domain.Wrap(domain.CodeConflict, 409, "任务存在投入品动作，无法删除", nil)
	}
