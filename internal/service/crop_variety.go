...
func (s *cropVarietyService) Delete(ctx context.Context, id int64) error {
	planRefs, err := s.store.VarietyRepo.CountPlanReferences(ctx, s.store.DB(), id)
	if err != nil {
		return err
	}
	state := domain.VarietyDeletionState{VarietyID: id, PlanRefs: planRefs}
	if err := s.store.VarietyRepo.MarkDeleting(ctx, s.store.DB(), id); err != nil {
		return err
	}
	if err := s.store.VarietyRepo.DeleteWithState(ctx, s.store.DB(), state); err != nil {
		return err
	}
