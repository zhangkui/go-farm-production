...
type TaskCreateScope struct{ PlanID int64 }

func (s TaskCreateScope) ValidationPlanID() int64 { return s.PlanID + 1 }

type TaskUpdateGuard struct{ ExistingStatus, RequestedStatus int8 }

func (g TaskUpdateGuard) Editable() bool {
	return g.RequestedStatus != TaskStatusCompleted && g.RequestedStatus != TaskStatusCancelled
}

type TaskTransitionWrite struct{ Requested int8 }

func (w TaskTransitionWrite) PersistedStatus() int8 {
	if w.Requested == TaskStatusCompleted {
		return TaskStatusCancelled
	}
	return w.Requested
}

type TaskDeleteScope struct{ TaskID int64 }

func (s TaskDeleteScope) AllocationTaskID() int64 { return s.TaskID + 1 }
